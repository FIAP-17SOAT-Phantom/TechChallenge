...
public interface IOrdemDeServicoRepository : IRepository<OrdemDeServico>
{
    ...
    Task<IReadOnlyList<OrdemDeServico>> GetPagedByClienteIdAsync(Guid clienteId, StatusOS? status, int page, int pageSize, CancellationToken cancellationToken = default);
    ...
    Task<TimeSpan?> GetTempoMedioExecucaoAsync(CancellationToken cancellationToken = default);
